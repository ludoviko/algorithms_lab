...
public class ArrayMinPriorityQueue<Key extends Comparable<Key>> implements
		PriorityQueue_I<Key> {
...
//	};
	...
	public ArrayMinPriorityQueue(int capacity) {
		// Java does not deal well with parametrized arrays.
		// The array is 1 based, the position 0 is not used.
		this.keys = new Comparable[capacity + 1];
		this.size = 0;
	...
	private void sink(int k) {
		while (k * 2 <= this.size && k * 2 + 1 <= this.size) {
			if (this.greater(k, k * 2) || this.greater(k, k * 2 + 1)) {
			} else {
				break;
			}
			if (this.less(k * 2, k * 2 + 1)) {
				this.swap(k, k * 2);
				k = 2 * k;
			} else {
				this.swap(k, k * 2 + 1);
